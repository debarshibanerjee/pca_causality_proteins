...
from ig_smoothing_common import (
    ...
    first_sustained_false_significance_time,
...
DEFAULT_SIGNIFICANCE_FORWARD_BEFORE_STEPS = 5
DEFAULT_SIGNIFICANCE_FORWARD_AFTER_STEPS = 5
DEFAULT_SIGNIFICANCE_REVERSE_BEFORE_STEPS = 2
DEFAULT_SIGNIFICANCE_REVERSE_AFTER_STEPS = 2
...
def integrate_pair(
    ...
    significance_forward_before_steps: int,
    significance_forward_after_steps: int,
    significance_reverse_before_steps: int,
    significance_reverse_after_steps: int,
    ...
    significance_cutoff_a_to_b = first_sustained_false_significance_time(
        taus,
        data[:, 3],
        before_steps=significance_forward_before_steps,
        after_steps=significance_forward_after_steps,
    )
    significance_cutoff_b_to_a = first_sustained_false_significance_time(
        taus,
        data[:, 4],
        before_steps=significance_reverse_before_steps,
        after_steps=significance_reverse_after_steps,
    )
    significance_cutoff_found_a_to_b = significance_cutoff_a_to_b is not None
    significance_cutoff_found_b_to_a = significance_cutoff_b_to_a is not None
    significance_effective_cutoff_a_to_b = significance_cutoff_a_to_b if significance_cutoff_found_a_to_b else float(taus[-1])
    significance_effective_cutoff_b_to_a = significance_cutoff_b_to_a if significance_cutoff_found_b_to_a else float(taus[-1])
    ...
    return (
        ...
        },
        {
            ...
            "tau_start_ns": float(taus.min()),
            "tau_stop_ns": float(taus.max()),
            "original_pc_a_to_pc_b_cutoff_found": significance_cutoff_found_a_to_b,
            "original_pc_b_to_pc_a_cutoff_found": significance_cutoff_found_b_to_a,
            "original_pc_a_to_pc_b_cutoff_ns": significance_effective_cutoff_a_to_b,
            "original_pc_b_to_pc_a_cutoff_ns": significance_effective_cutoff_b_to_a,
            "original_pc_a_to_pc_b_area_to_cutoff": integrate_positive_curve_up_to(
                taus,
                data[:, 1],
                significance_effective_cutoff_a_to_b,
                method=integration_method,
            ),
            "original_pc_b_to_pc_a_area_to_cutoff": integrate_positive_curve_up_to(
                taus,
                data[:, 2],
                significance_effective_cutoff_b_to_a,
                method=integration_method,
            ),
            "smoothed_pc_a_to_pc_b_cutoff_found": significance_cutoff_found_a_to_b,
            "smoothed_pc_b_to_pc_a_cutoff_found": significance_cutoff_found_b_to_a,
            "smoothed_pc_a_to_pc_b_cutoff_ns": significance_effective_cutoff_a_to_b,
            "smoothed_pc_b_to_pc_a_cutoff_ns": significance_effective_cutoff_b_to_a,
            "smoothed_pc_a_to_pc_b_area_to_cutoff": integrate_positive_curve_up_to(
                smooth_a_to_b[:, 0],
                smooth_a_to_b[:, 1],
                significance_effective_cutoff_a_to_b,
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=("Integrate pairwise IG curves from original IG-*.txt files and from the matching smoothed curves.")
    )
    parser.add_argument(
        "input_dir",
        nargs="?",
        # default="analysis_outputs/ig_results/E=1_k=50_dci=0",
        default="analysis_outputs/ig_results/r2-70_E=1_k=50_dci=0",
    ...
    parser.add_argument(
        "--significance-cutoff-csv-name",
        default="ig_curve_integrals_cutoff_significance.csv",
        help="Filename for the significance-cutoff CSV output written inside input_dir.",
    )
    parser.add_argument(
        "--significance-cutoff-json-name",
        default="ig_curve_integrals_cutoff_significance.json",
        help="Filename for the significance-cutoff JSON output written inside input_dir.",
    )
    parser.add_argument(
        "--significance-forward-before-steps",
        type=int,
        default=DEFAULT_SIGNIFICANCE_FORWARD_BEFORE_STEPS,
        help="Required false significance-marker steps before the forward cutoff point.",
    )
    parser.add_argument(
        "--significance-forward-after-steps",
        type=int,
        default=DEFAULT_SIGNIFICANCE_FORWARD_AFTER_STEPS,
        help="Required false significance-marker steps after the forward cutoff point.",
    )
    parser.add_argument(
        "--significance-reverse-before-steps",
        type=int,
        default=DEFAULT_SIGNIFICANCE_REVERSE_BEFORE_STEPS,
        help="Required false significance-marker steps before the reverse cutoff point.",
    )
    parser.add_argument(
        "--significance-reverse-after-steps",
        type=int,
        default=DEFAULT_SIGNIFICANCE_REVERSE_AFTER_STEPS,
        help="Required false significance-marker steps after the reverse cutoff point.",
    )
    parser.add_argument(
        "--interp-points",
        type=int,
...
def main() -> int:
    ...
    main_rows = []
    cutoff_rows = []
    significance_cutoff_rows = []
    for ig_path in ig_paths:
        main_row, cutoff_row, significance_cutoff_row = integrate_pair(
            input_path=ig_path,
            ...
            significance_forward_before_steps=args.significance_forward_before_steps,
            significance_forward_after_steps=args.significance_forward_after_steps,
            significance_reverse_before_steps=args.significance_reverse_before_steps,
            significance_reverse_after_steps=args.significance_reverse_after_steps,
        )
        main_rows.append(main_row)
        cutoff_rows.append(cutoff_row)
        significance_cutoff_rows.append(significance_cutoff_row)
    ...
    significance_cutoff_csv_path = input_dir / args.significance_cutoff_csv_name
    significance_cutoff_json_path = input_dir / args.significance_cutoff_json_name
    write_main_csv(csv_path, main_rows)
    write_cutoff_csv(cutoff_csv_path, cutoff_rows)
    write_cutoff_csv(significance_cutoff_csv_path, significance_cutoff_rows)
    write_json(
        json_path,
        main_rows,
        input_dir=input_dir,
    ...
    write_json(
        ...
        extra_metadata={
            "cutoff_rule": (
                "For each direction, compute an area truncated at the first post-onset time where the "
                "original IG significance marker is false, the requested number of previous markers are "
                "also false, and the requested number of following markers are also false."
            ),
            "significance_cutoff_parameters": {
                "forward_before_steps": args.significance_forward_before_steps,
                "forward_after_steps": args.significance_forward_after_steps,
                "reverse_before_steps": args.significance_reverse_before_steps,
                "reverse_after_steps": args.significance_reverse_after_steps,
            },
            "significance_marker_rule": "Original IG significance markers are treated as true when marker > 0.",
        },
    )
    ...
    print(significance_cutoff_csv_path)
    print(significance_cutoff_json_path)
    return 0
