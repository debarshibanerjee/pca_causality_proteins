...
def first_sustained_false_significance_time(
    x_values: np.ndarray,
    ...
    """
    Find the first post-onset non-significant time with a stable false neighborhood.

    Significance values are treated as true when they are > 0. The search starts
    after at least one significant point has appeared, then looks for a false
    marker whose previous before_steps markers and next after_steps markers are
    also false.
    """
    if before_steps < 0 or after_steps < 0:
        raise ValueError("before_steps and after_steps must be non-negative.")

    is_significant = np.asarray(significance_values) > 0.0
    significant_indices = np.flatnonzero(is_significant)
    if significant_indices.size == 0:
        return None

    first_candidate = max(int(significant_indices[0]) + 1, before_steps)
    last_candidate = len(is_significant) - after_steps - 1
    for idx in range(first_candidate, last_candidate + 1):
        if is_significant[idx]:
            continue
        previous_start = idx - before_steps
        next_stop = idx + after_steps + 1
        if is_significant[previous_start:idx].any():
            continue
        if is_significant[idx + 1 : next_stop].any():
            continue
