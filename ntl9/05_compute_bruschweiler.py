...
def parse_ca_residues(topology_path: Path, n_ca: int) -> pd.DataFrame:
...
def bruschweiler_collectivity(components: np.ndarray, eps: float) -> dict[str, np.ndarray]:
    ...
    entropy_terms = np.zeros_like(contributions)
    nonzero = contributions > eps
    entropy_terms[nonzero] = contributions[nonzero] * np.log(contributions[nonzero])
...
def parse_args() -> argparse.Namespace:
    ## 830to1060us defaults:
    default_components = "analysis_outputs/pca_sklearn_standard/830to1060us_sklearn_standard_components.npy"
    default_spectrum = "analysis_outputs/pca_sklearn_standard/830to1060us_sklearn_standard_spectrum.csv"
    default_output_dir = "analysis_outputs/collectivity/830to1060us_sklearn_standard"
    default_label = "830to1060us_sklearn_standard"

    ## 200to490us alternate defaults:
    # default_components = "analysis_outputs/pca_sklearn_standard/200to490us_sklearn_standard_components.npy"
    # default_spectrum = "analysis_outputs/pca_sklearn_standard/200to490us_sklearn_standard_spectrum.csv"
    # default_output_dir = "analysis_outputs/collectivity/200to490us_sklearn_standard"
    # default_label = "200to490us_sklearn_standard"
...
def main() -> None:
    ...
    if topology_path is None:
        ...
        residue_table = parse_ca_residues(topology_path, n_ca)
