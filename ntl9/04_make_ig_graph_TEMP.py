...
# DEFAULT_DATA_DIR = Path("analysis_outputs/ig_results/200to490us_sklearn_standard_top5/E=1_k=50_dci=0")
DEFAULT_DATA_DIR = Path("analysis_outputs/ig_results/830usto1060us_sklearn_standard_top5/E=1_k=50_dci=0")
DEFAULT_NUM_PCS = 5
...
# Other options for ig_curve_integrals_cutoff.csv:
...
OUTPUT_NAME_WITH_VALUES = "PC_Integral_Network_Graph_color_length_dotted_smoothed_full_only.png"
OUTPUT_NAME_WITHOUT_VALUES = "PC_Integral_Network_Graph_color_length_dotted_smoothed_full_only_no_values.png"
...
def print_normalized_integrals(directed_records: list[tuple[int, int, float]]) -> None:
    normalized_records = normalize_records(directed_records)
    max_value = max(raw_value for _, _, raw_value, _ in normalized_records)

    print(f"Normalized smoothed full-integral values (max integral = {max_value:.10g}):")
    for pair_start in range(0, len(normalized_records), 2):
        pair_records = normalized_records[pair_start : pair_start + 2]
        if len(pair_records) == 2:
            source_a, target_a, _, value_a = pair_records[0]
            source_b, target_b, _, value_b = pair_records[1]
            if source_a == target_b and target_a == source_b:
                print(
                    f"PC{source_a}-PC{target_a}: "
                    f"PC{source_a}->PC{target_a}={value_a:.6f}, "
                    f"PC{source_b}->PC{target_b}={value_b:.6f}"
                )
                continue

        for source, target, _, normalized_value in pair_records:
            print(f"PC{source}->PC{target}: {normalized_value:.6f}")
...
def main() -> int:
    ...
    print_normalized_integrals(directed_records)
